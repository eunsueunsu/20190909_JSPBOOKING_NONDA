...
/**
 * Servlet implementation class ProgramUpdateServlet
 */
@WebServlet("/pUpdate.pr")
...
//		String hdPoster = mrequest.getParameter("hdPoster");
//		System.out.println("hdPoster: " + hdPoster);
//		String hdDetail = request.getParameter("hdDetail");
		...
		int pcode = Integer.parseInt(mrequest.getParameter("pno"));
		...
		try {
			...
			if(pposter == null) { 
				pposter = mrequest.getParameter("hdPoster"); 
				System.out.println("pposter이 null임.");
				System.out.println("pposter값 새로설정: " + pposter);
			}
			
			if(pdetailimg == null) { 
				pposter = mrequest.getParameter("hdDetail"); 
			}
			...
			System.out.println("서비스 결과 처리하기 전");

			// 7. 서비스 결과 처리하기
			int result = new ProgramService().updateProgram(p,pposter,pdetailimg);

			if(result > 0) {
				// 수정 완료되면 detail페이지로 이동
				response.sendRedirect("/Nonda/pRequestDetail.pr?pno=" + pcode);
			} else {
